package application
{
	...
	public class ApplicationClass extends Application
	{
		...
		private function setProtocols():void
		{
			...
			//Display deletions, if there are any
			displayDeletions();
		}
		...
		//Display any deleted URLs to the user
		private function displayDeletions():void{
			//If any URLs are deleted as a result of a change in the accepted protocols/extensions, show a message
			if (urlSet.deletedUrls.length != 0)
			{
				//Iterate through deleted URLs, and add them to a string as a list
				var urlList:String = "";
				for each(var url:String in urlSet.deletedUrls)
				{
					urlList =  urlList + url + '\n';
				}
				//Display Message To User
				Alert.show(urlList, String(urlSet.deletedUrls.length + " URLs Deleted"));
			}
		}
